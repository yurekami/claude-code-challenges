...
export default function Terminal() {
      ...
      case 'start':
        if (args[0] && currentCategory) {
          ...
          if (num > 0 && num <= currentCategory.challenges.length) {
            startChallenge(currentCategory.challenges[num - 1]);
          } else {
  ...
  const listChallenges = (cat: Category) => {
      ...
      addOutput(
        <div className="border border-[#30363D] p-4 rounded my-2">
          ...
          <div className="text-[#58A6FF]">
            Challenge loaded! Open /challenges/{currentCategory?.slug}/{ch.slug}/ in your editor.
          </div>
        </div>
      );
      addOutput('');
